...
const LinesEditor = (props) => {
    ...
    const [savedNotes, setSavedNotes] = useState(JSON.parse(localStorage.getItem(`saved-notes-line${selectedNumber}`)) || {});
    const [visible, setVisible] = useState(visibleLifted);
    const [goal, setGoal] = useState(JSON.parse(localStorage.getItem(`goalProgress-line${selectedNumber}-${notePath}`))?.goal);
    const [progress, setProgress] = useState(JSON.parse(localStorage.getItem(`goalProgress-line${selectedNumber}-${notePath}`))?.progress);
    ...
    useEffect(() => {
        ...
        const storedData = JSON.parse(localStorage.getItem(`goalProgress-line${selectedNumber}-${row?.id}`)); // Assuming `row.id` is unique
    ...
    useEffect(() => {
        setSavedNotes(JSON.parse(localStorage.getItem(`saved-notes-line${selectedNumber}`)) || {})
    },[selectedNumber]);
        ...
        if (Array.isArray(savedNotes[noteId][rowIndex])) {
            ...
            if (newNote && newNote.noteId) {
                ...
                if (!noteExists) {
                    ...
                    spMethod.handleSubmit(noteId, JSON.stringify(savedNotes), `line${selectedNumber}`, 'NOTES').then(e => console.log(e)).catch(err => console.log(err));
    ...
    const saveFile = () => {
        try {
            ...
            XLSX.writeFile(wb, 'line_Open_This_Lastest_File.xlsx');
        } catch (error) {
    ...
    const openPdfModal = (pdfFile) => {
        //setPdfPath(pdfFile);
        $('.ui.fullscreen.modal.pdf-viewer.line').modal('show'); // Show the PDF modal
    };
    ...
    const openNoteModal = () => {
        $('.ui.small.modal.note-viewer.line').modal('show'); // Use jQuery to show the modal 
    }

    const openModal = () => {
        $('.addRecord.small.modal.line').modal('show'); // Use jQuery to show the modal
    };
    ...
    console.log('line Pane', departmentName, dataLifted);
    ...
    return React.createElement('div', { className: 'ui container grid ', style: { marginTop: '20px' } },
        ...
        React.createElement('div', { className: 'ui fullscreen modal pdf-viewer line' },
        ...
        React.createElement('div', { className: 'ui small modal note-viewer line' },
