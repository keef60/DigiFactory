...
const FramesEditor = (props) => {
  ...
  /*   const [data, setData] = useState([]); */
    ...
    const [savedNotes, setSavedNotes] = useState(JSON.parse(localStorage.getItem('saved-notes-frames')) || {});
    const [visible, setVisible] = useState(false);
    const [goal, setGoal] = useState(JSON.parse(localStorage.getItem(`goalProgress-frames-${notePath}`))?.goal);
    const [progress, setProgress] = useState(JSON.parse(localStorage.getItem(`goalProgress-frames-${notePath}`))?.progress);
    ...
    useEffect(() => {
        localStorage.setItem('notes-frames', JSON.stringify(notes));
    ...
    $(document).on('click', '.save-new-record-frames', function () {
    ...
    $(document).on('click', '.save-note-frames', function (e) {
    ...
    const saveNoteToLocalStorage = (noteId, rowIndex, newNote) => {
        let savedNotes = JSON.parse(localStorage.getItem('saved-notes-frames')) || {};
    ...
    const openPdfModal = (pdfFile) => {
        //setPdfPath(pdfFile);
        $('.ui.fullscreen.modal.pdf-viewer.frames').modal('show'); // Show the PDF modal
    };
    ...
    const openNoteModal = () => {
        $('.ui.small.modal.note-viewer.frames').modal('show'); // Use jQuery to show the modal 
    }

    const openModal = () => {
        $('.addRecord.small.modal.frames').modal('show'); // Use jQuery to show the modal
    };
    ...
    return React.createElement('div', { className: 'ui  grid ', style: { marginTop: '20px' } },
        ...
        React.createElement('div', { className: 'ui small modal addRecord frames' },
            ...
            React.createElement('div', { className: 'actions' },
                ...
                React.createElement('button', {
                    className: 'ui primary button approve save-new-record-frames',
                }, 'Add')
            )
        ),

        // **Modal for viewing pdf**
        React.createElement('div', { className: 'ui fullscreen modal pdf-viewer frames' },
        ...
        React.createElement('div', { className: 'ui small modal note-viewer frames' },
                            ...
                            React.createElement('div', { className: 'ui comments' },
                                React.createElement('div', { className: 'comment' },
                                    React.createElement('div', { className: 'content' },
                                        ...
                                        React.createElement('button', {
                                            className: 'ui primary labeled icon button save-note-frames',
