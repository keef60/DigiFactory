...
const ProductionDowntimeReport = ({ spMethod }) => {
  ...
  useEffect(() => {
    const fetchDowntimeData = async () => {
      try {
        ...
        const issues = await spMethod.fetchSharePointData('ISSUES', 'issues', false)
          .then(e => e.value)
        ...
        // Loop through the SharePoint issues and structure the data
          ...
          const issueWithThisOrder = JSON.parse(issue.fields[departmentClick]).selectedOptions
